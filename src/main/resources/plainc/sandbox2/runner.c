...
int main(void)
{
	...
	sin(int) *v=new_sin(int);//=new_sin(int);

	_class *test=foo(int);

	//sin(int) *v = sin_new(int);
	//mem(int) *p = new_mem(int);
	//mem(float) *kai=new_mem(float);

	//printf("%d",INT);
	//foo(9);
	//printf("%d",getTypeOf(9));
	...
	_ini:
		...
		//_MM = (struct _mem **) malloc (1 * sizeof(struct _mem));
